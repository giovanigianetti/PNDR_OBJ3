...
function colorFor(value: number | null, metric: MetricKey, min: number, max: number) {
  ...
  const diverging = ["popPct", "agingPct", "olderSharePp"].includes(metric);
  if (diverging) {
    if (value < 0) return blend("#9d4258", "#f3edf0", Math.min(1, value / Math.min(min, -0.0001)));
    ...
    <div className="map-wrap" ref={holder}>
      ...
      {hover && (
        <div className="map-tooltip" style={{ left: Math.min(hover.x + 12, width - 245), top: hover.y + 10 }}>
          ...
          <b>{metricLabels[metric]}: {metric === "pop2022" ? integerNumber.format(getMetric(hover.row, metric) ?? 0) : fmt(getMetric(hover.row, metric), metric === "olderSharePp" ? " p.p." : "%")}</b>
          <small>{hover.row.dynamism}</small>
        </div>
      )}
      <div className="map-legend">
        <span>{metric === "pop2022" ? compactNumber.format(min) : fmt(min)}</span>
        <i className={['popPct', 'agingPct', 'olderSharePp'].includes(metric) ? "legend-diverging" : "legend-sequential"} />
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("./data/dashboard_obj3.json").then((response) => response.json()),
      fetch("./data/validation.json").then((response) => response.json()),
      fetch("./data/municipios_ibge_topo.json").then((response) => response.json()),
      fetch("./data/rgi_geo.json").then((response) => response.json()),
    ])
      .then(([dashboard, checks, topology, rgi]) => {
        setData(dashboard);
        setValidation(checks);
        const converted = feature(topology, topology.objects.BRMU) as unknown as FeatureCollection;
  ...
  const scatterGroups = useMemo(() => {
    ...
    return Object.keys(dynamismColors).map((name) => ({
      name,
      data: territories.filter((row) => row.dynamism === name).map(toPoint),
    }));
  }, [territories]);
  ...
  function downloadCsv() {
    const header = ["codigo", "territorio", "uf", "rgi", "tipologia", "dinamismo", "pop_2010", "pop_2022", "variacao_pop_pct", "indice_60_2010", "indice_60_2022", "variacao_indice_pct", "idosos_2022_pct", "diferenca_idosos_pp"];
      ...
      <section className="dashboard-shell">
        <aside className="filters">
          ...
          <label className="filter-field">
            ...
            <select value={level} onChange={(event) => { setLevel(event.target.value as "municipality" | "rgi"); setSelectedCode(""); }}>
